    ...
    <div className="w-full bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="p-6 sm:p-8 bg-gradient-to-b from-slate-50 to-white border-b border-slate-100">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="grid grid-cols-2 gap-2 bg-slate-100/80 p-3 rounded-xl border border-slate-200 text-xs shrink-0 md:w-64">
            <div>
              <div className="text-[10px] uppercase font-semibold text-slate-400">Completed</div>
              <div className="font-bold text-slate-800 truncate">{recognition.yearCompletedOrPeriod}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase font-semibold text-slate-400">Builder/Architect</div>
              <div className="font-bold text-slate-800 truncate">{recognition.architectOrBuilder}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase font-semibold text-slate-400">AI Confidence</div>
              <div className="font-bold text-emerald-600">{recognition.confidenceScore}% Vision Match</div>
            </div>
            <div>
              <div className="text-[10px] uppercase font-semibold text-slate-400">Grounding</div>
              <div className="font-bold text-blue-600 truncate">Google Search Live</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Navigation tabs */}
      <div className="flex border-b border-slate-200 px-6 bg-slate-50/50 gap-6 text-sm font-medium">
        <button
      ...
      <div className="p-6 sm:p-8">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 max-w-4xl">
            <div>
              <h3 className="text-lg font-bold text-slate-900 mb-2">The Origin Story</h3>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {history.visitorTips.map((tip, idx) => (
                    <div
                      key={idx}
                      className="flex items-start gap-2.5 p-3.5 rounded-xl bg-amber-50/70 border border-amber-200/60 text-xs sm:text-sm text-amber-950"
                    >
        ...
        {activeTab === 'timeline' && (
          <div className="relative pl-6 sm:pl-8 border-l-2 border-amber-400/80 space-y-8 max-w-3xl">
            {history.historicalTimeline.map((item, idx) => (
              <div key={idx} className="relative group">
                {/* Node icon */}
                <div className="absolute -left-[31px] sm:-left-[39px] top-1 w-6 h-6 rounded-full bg-amber-500 text-white flex items-center justify-center font-bold text-xs ring-4 ring-white shadow-sm">
                  {idx + 1}
                </div>
                <div>
        ...
        {activeTab === 'secrets' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {history.hiddenSecrets.map((secret, idx) => (
              <div
                key={idx}
                className="bg-slate-50 rounded-xl p-5 border border-slate-200 hover:border-amber-300 hover:bg-amber-50/30 transition-all flex flex-col justify-between"
              >
                <div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-200/60 text-[10px] font-mono text-slate-400">
                  SECRET #{idx + 1}
                </div>
              </div>
            ))}
        ...
        {activeTab === 'sources' && (
          <div className="space-y-4 max-w-2xl">
            ...
            {history.searchSources.length === 0 ? (
              <p className="text-xs text-slate-500">No external source links retrieved.</p>
            ) : (
              <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden">
                {history.searchSources.map((source, idx) => (
                  <a
                    ...
                    className="p-3.5 flex items-center justify-between hover:bg-slate-50 transition-colors group block"
                  >
                    ...
                    <span className="text-[11px] text-blue-600 font-medium shrink-0 ml-2 group-hover:translate-x-0.5 transition-transform">
                      Open &rarr;
                    </span>
                  </a>
                ))}
