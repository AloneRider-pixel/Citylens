...
export const LandmarkChallengeQuiz: React.FC<LandmarkChallengeQuizProps> = ({
  ...
  return (
    <section className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mt-6">
      {/* Quiz Section Header */}
      <div className="p-4 sm:px-6 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 bg-gradient-to-r from-amber-500/10 via-slate-50 to-indigo-500/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-slate-900 text-base">Landmark Challenge</h3>
              <span className="text-[10px] font-mono font-bold uppercase tracking-wider bg-amber-500/20 text-amber-900 border border-amber-500/30 px-2 py-0.5 rounded-full">
                3-Question Quiz
              </span>
            </div>
        ...
        {hasStarted && !isCompleted && (
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-bold text-slate-600 bg-white border border-slate-200 px-3 py-1 rounded-lg shadow-2xs">
      ...
      <div className="p-4 sm:p-6">
        {isLoading ? (
          ...
          <div className="py-8 px-2 max-w-xl mx-auto text-center space-y-5">
            <div className="w-16 h-16 rounded-2xl bg-amber-100 border border-amber-300 text-amber-700 mx-auto flex items-center justify-center shadow-inner">
            ...
            <div className="grid grid-cols-3 gap-2 text-center py-2 max-w-sm mx-auto">
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                <span className="block font-bold text-slate-900 text-sm">3</span>
                <span className="text-[10px] text-slate-500 uppercase font-semibold">Questions</span>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                <span className="block font-bold text-slate-900 text-sm">~1 min</span>
                <span className="text-[10px] text-slate-500 uppercase font-semibold">Duration</span>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                <span className="block font-bold text-slate-900 text-sm">Passport</span>
                <span className="text-[10px] text-slate-500 uppercase font-semibold">Stamp Reward</span>
              </div>
            </div>
              ...
              return (
                <div className="text-center space-y-4">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {isCorrect ? (
                              <span className="shrink-0 flex items-center gap-1 font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md">
                                <CheckCircle2 className="w-3.5 h-3.5" /> Correct
                              </span>
                            ) : (
                              <span className="shrink-0 flex items-center gap-1 font-bold text-rose-700 bg-rose-100 px-2 py-0.5 rounded-md">
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                    <button
                      onClick={loadQuiz}
                      className="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
                    >
            ...
            <div className="max-w-xl mx-auto space-y-5">
              ...
              <div className="space-y-1.5">
                {currentQuestion.historicalContextSnippet && (
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md inline-block">
                    {currentQuestion.historicalContextSnippet}
                  </span>
                )}
              ...
              <div className="space-y-2.5">
                {currentQuestion.options.map((option, optIdx) => {
                  ...
                  let cardStyle = 'bg-white hover:bg-slate-50 border-slate-200 text-slate-800 shadow-2xs';
                  let letterBadge = 'bg-slate-100 text-slate-700 border-slate-300';

                  if (isAnswerSubmitted) {
                    if (isCorrect) {
                      cardStyle = 'bg-emerald-50 border-emerald-400 text-emerald-950 ring-2 ring-emerald-400/20';
                      letterBadge = 'bg-emerald-500 text-white border-emerald-600';
                    } else if (isSelected) {
                      cardStyle = 'bg-rose-50 border-rose-400 text-rose-950 ring-2 ring-rose-400/20';
              ...
              {isAnswerSubmitted && (
                <div className="flex justify-end pt-2">
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs flex items-center gap-1.5 shadow-sm transition-all hover:translate-x-0.5 cursor-pointer"
                  >
                    <span>
